...
check($certificate['recipient_masked_name'] === 'Ágata ***** Oliveira', 'First and third names visible');
foreach (['' => 'Identificação preservada', 'Ana' => 'Ana', 'Ana Silva' => 'Ana *****', 'Ana Maria Silva Souza' => 'Ana ***** Silva *****', "  Ana   Maria\tSilva  " => 'Ana ***** Silva'] as $name => $expected) {
    $result = CertificatePublicData::project(array_merge($source, ['student_name' => $name]));
    check($result['recipient_masked_name'] === $expected, 'Name masking: ' . $name);
}
...
ob_start(); require dirname(__DIR__) . '/app/Views/public/certificate-verify.php'; $html = ob_get_clean();
check(!str_contains($html, 'Ágata') && str_contains($html, 'Certificado revogado'), 'Revoked view preserves privacy');
$certificate = null; $code = '';
ob_start(); require dirname(__DIR__) . '/app/Views/public/certificate-verify.php'; ob_end_clean();
echo "Verificação: nomes parcialmente ocultos, estados privados e escape de HTML aprovados.\n";
