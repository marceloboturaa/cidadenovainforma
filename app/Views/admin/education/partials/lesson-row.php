...
?>
<article class="education-playlist-row <?= $lessonScheduleLocked ? 'is-scheduled' : '' ?>">
    <<?= $lessonAccessLocked ? 'div' : 'a' ?> class="education-playlist-main"<?= $lessonAccessLocked ? '' : ' href="' . e(url('/admin/education/lesson?id=' . $lesson['id'])) . '"' ?>>
        <span class="<?= !empty($lesson['completed_at']) ? 'is-complete' : '' ?>">
            <i class="bi <?= $lessonAccessLocked ? 'bi-lock-fill' : (!empty($lesson['completed_at']) ? 'bi-check-circle-fill' : 'bi-circle') ?>" aria-hidden="true"></i>
        </span>
        <span>
    ...
    <div class="education-lesson-actions">
        ...
        <?php else: ?>
            <a class="btn btn-sm btn-primary icon-btn" href="<?= e(url('/admin/education/lesson?id=' . $lesson['id'])) ?>">
                <i class="bi bi-play-circle" aria-hidden="true"></i>Assistir
            </a>
        <?php endif; ?>
        <?php if ($canManage): ?>
            <?php if ($lessonAttendanceMode === 'manual'): ?>
