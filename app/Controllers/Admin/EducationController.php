...
class EducationController
{
        ...
        foreach ($report['students'] as $student) {
            ...
            foreach ($activities as $activity) {
                ...
                fputcsv($output, [
                    ...
                    $student['name'] ?? '',
                    $student['email'] ?? '',
                    !empty($student['needs_attention']) ? 'Requer acompanhamento' : 'Em dia',
                    ...
                    $activity ? (($activity['type'] ?? '') === 'form' ? 'Formulário' : 'Tarefa') : '',
                    $activity['title'] ?? '',
    ...
    public function updateCertificate(): void
    {
        ...
        Education::updateCourse((int) $course['id'], array_merge($course, [
            ...
            'certificate_show_text' => !empty($_POST['certificate_show_text']) ? 1 : 0,
            'certificate_show_qr' => !empty($_POST['certificate_show_qr']) ? 1 : 0,
            ...
            'certificate_show_period' => !empty($_POST['certificate_show_period']) ? 1 : 0,
            'certificate_show_approval' => !empty($_POST['certificate_show_approval']) ? 1 : 0,
            'certificate_show_institution' => !empty($_POST['certificate_show_institution']) ? 1 : 0,
            'certificate_show_meta' => !empty($_POST['certificate_show_meta']) ? 1 : 0,
            'certificate_show_legal' => !empty($_POST['certificate_show_legal']) ? 1 : 0,
            ...
            'certificate_footer_on_back' => !empty($_POST['certificate_footer_on_back']) ? 1 : 0,
            'certificate_hide_responsible' => !empty($_POST['certificate_hide_responsible']) ? 1 : 0,
            'certificate_program_background_enabled' => !empty($_POST['certificate_program_background_enabled']) ? 1 : 0,
            'certificate_program_text_color' => $_POST['certificate_program_text_color'] ?? null,
            'certificate_program_background_color' => $_POST['certificate_program_background_color'] ?? null,
    ...
    private function coursePublicMode(?array $course): string
    {
        ...
        return !empty($course['public_access_enabled']) ? 'mixed' : 'private';
    }

    private function courseIsPublic(?array $course): bool
    {
        return !empty($course['public_enabled'])
            && $this->coursePublicMode($course) !== 'hidden'
            && ($course['certificate_activity_type'] ?? 'curso_livre') !== 'reconhecimento';
    }
    ...
    private function certificateFieldsFromCourse(array $course): array
    {
        return [
            ...
            'certificate_show_text' => $course['certificate_show_text'] ?? 1,
            'certificate_show_qr' => $course['certificate_show_qr'] ?? 1,
            ...
            'certificate_footer_on_back' => $course['certificate_footer_on_back'] ?? 0,
            'certificate_hide_responsible' => $course['certificate_hide_responsible'] ?? 0,
            'certificate_program_background_enabled' => $course['certificate_program_background_enabled'] ?? 1,
            'certificate_program_text_color' => $course['certificate_program_text_color'] ?? null,
            'certificate_program_background_color' => $course['certificate_program_background_color'] ?? null,
    ...
    private function courseVisibilityFromRequest(): array
    {
        ...
        return match ($mode) {
            ...
            'private' => ['public_enabled' => 1, 'public_access_enabled' => 0, 'public_access_mode' => 'private'],
            default => ['public_enabled' => 0, 'public_access_enabled' => 0, 'public_access_mode' => 'hidden'],
        };
    }
